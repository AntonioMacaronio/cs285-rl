...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        action_dim: int,
        make_actor: Callable[[Tuple[int, ...], int], nn.Module], # this predicts the index of maximum
    ...
    def update_critic(
        ...
        action: torch.Tensor,   # [batch_size, ac_dim]
        ...
        with torch.no_grad():
            ...
            next_action_distribution = self.actor(next_obs)     # torch.distributions.Distribution
            next_action = next_action_distribution.sample()     # [batch_size, ac_dim]
            next_qs = self.target_critic(next_obs, next_action) # [num_critics, batch_size] which represent Q(s', a')
            # ENDTODO
            ...
            target_values = reward + self.discount*(1.0 - done.float()) * next_qs # [num_critics, batch_size]
            # ENDTODO
        ...
        # Predict Q-values
        q_values = self.critic(obs, action) # [num_critics, batch_size] representing Q(s, a)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        sample = action_distribution.rsample()  # [batch_size, ac_dim]
        return -action_distribution.log_prob(sample) # [batch_size, ]
        # ENDTODO
